...
def _crossref(question: str) -> list[dict[str, Any]]:
    ...
    request = urllib.request.Request(
        url,
        headers={"User-Agent": "cs-nature-paper-provider/4.0.0 (mailto:noreply@example.invalid)"},
    )
...
def _retrieve_open_text(project: Path, source: dict[str, Any], index: int) -> dict[str, Any]:
    ...
    request = urllib.request.Request(str(url), headers={"User-Agent": "cs-nature-paper-provider/4.0.0"})
    try:
