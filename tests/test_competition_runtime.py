...
class CompetitionInitializationTests(unittest.TestCase):
    ...
    def test_competition_mode_selects_contest_graph_and_state(self):
        ...
        policy = self.read("autonomy_policy.json")
        self.assertTrue(policy["permissions"]["network"])
        self.assertTrue(policy["permissions"]["auto_hire"])
...
class CompetitionSchedulerTests(unittest.TestCase):
    ...
    def test_temporary_policy_blocks_are_overlay_only(self):
        self.set_ready("model_improvement", "revision")
        ...
        self.assertTrue(
            any(
                item["node"] == "model_improvement" and item["policy_blocked"]
                for item in scheduled["blocked"]
            )
        )
        self.assertEqual(scheduled["policy_actions"], [])
        ...
        self.assertEqual(by_id["model_improvement"]["status"], "READY")
        self.assertNotEqual(by_id["revision"]["status"], "PASS")
        self.assertEqual(advanced["changed"], [])
        self.assertEqual(graph_path.read_bytes(), before)

    def test_execute_next_uses_shared_authorization_executor_evidence_and_graph(self):
        result = competition_runtime.execute_next(self.project, now_utc=self.START)
        self.assertEqual(result["status"], "PASS")
        self.assertEqual(result["authorization"]["status"], "AUTHORIZED")
        self.assertTrue(result["execution"]["artifacts"])
        graph = json.loads((self.state / "research_graph.json").read_text(encoding="utf-8"))
        self.assertEqual(graph["nodes"][0]["status"], "PASS")
        ledger = json.loads((self.state / "evidence_ledger.json").read_text(encoding="utf-8"))
        self.assertTrue(ledger["anchors"])
        self.assertTrue((self.state / ".autonomy-audit.jsonl").exists())

    def test_problem_selection_is_auto_only_when_the_best_choice_is_clear(self):
        candidates = [
            {"id": "A", "completion_risk": 0.1, "data_risk": 0.1, "model_risk": 0.1, "paper_potential": 0.9, "resource_risk": 0.1, "expected_workload": 0.2},
            {"id": "B", "completion_risk": 0.5, "data_risk": 0.4, "model_risk": 0.4, "paper_potential": 0.5, "resource_risk": 0.4, "expected_workload": 0.6},
        ]
        clear = competition_runtime.select_problem(candidates)
        close = competition_runtime.select_problem([candidates[0], candidates[0] | {"id": "C", "paper_potential": 0.89}])
        self.assertEqual((clear["decision"], clear["selected_problem"]), ("AUTO", "A"))
        self.assertEqual(close["decision"], "ASK_AUTHOR")
        self.assertTrue(clear["selection_rationale"])
...
class CompetitionMethodRouterTests(unittest.TestCase):
    ...
    def test_hybrid_problem_keeps_multiple_candidate_families_then_reasons(self):
        result = self.router.route("predict demand and optimize vehicle routing under capacity constraints")
        self.assertIn("prediction", result["candidate_families"])
        self.assertIn("optimization", result["candidate_families"])
        self.assertEqual(result["routing_stage"], "CANDIDATE_FAMILY_DETECTION")
        self.assertTrue(result["scientific_reasoning"]["baseline"])
        self.assertTrue(result["scientific_reasoning"]["primary_model"])
        self.assertTrue(result["scientific_reasoning"]["validation_plan"])
