...
[Chinese](README_zh.md) | [v3 branch](https://github.com/KaiserIIII/cs-nature-paper-skill/tree/v3) | [v2 branch](https://github.com/KaiserIIII/cs-nature-paper-skill/tree/v2) | [v1 branch](https://github.com/KaiserIIII/cs-nature-paper-skill/tree/v1) | [MIT License](LICENSE)

## What V3 adds

V3 keeps V2's claim/evidence/mechanism kernel and V1's execution-oriented CEO,
then adds:
...
- formal `claim -> result -> data -> command -> commit -> environment -> hash`
  anchors and a V2-preserving migration command.

## Scientific boundaries retained

The seven departments remain adaptive capability contracts. V3 never invents
...
Review third-party skill code and pin the version you intend to use. V3 does
...
Initialize the private V3 control plane:
...
To migrate an existing V2 project without deleting its state:

```bash
python scripts/research_state.py migrate-v2 /path/to/project
```

V2 remains in `.research-state`; V3 is copied to `.research-state-v3`.
