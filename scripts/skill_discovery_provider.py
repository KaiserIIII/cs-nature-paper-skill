...
class GitHubPublicBackend:
    ...
    def _get(self, url: str) -> Any:
        request = urllib.request.Request(
            url,
            headers={"Accept": "application/vnd.github+json", "User-Agent": "cs-nature-paper-provider/4.1.0"},
        )
    ...
    def _audit_files(self, repo: str, exact_ref: str) -> dict[str, str]:
        ...
        for path in selected[:40]:
            ...
            request = urllib.request.Request(raw, headers={"User-Agent": "cs-nature-paper-provider/4.1.0"})
            try:
