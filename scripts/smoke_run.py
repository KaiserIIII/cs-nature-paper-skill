...
def _fill_state(project: Path) -> None:
    ...
    _write(state / "claims.json", {"schema_version": 1, "skill_version": SKILL_VERSION, "claims": [{"id": "C1", "text": "The public fixture records one deterministic repair outcome.", "type": "descriptive", "scope": "public fixture only", "required_evidence": "observed fixture output", "observed_evidence": [], "status": "SCOPED"}]})
...
def run(output: Path | None = None) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="cs-nature-v4-smoke-") as directory:
        ...
        fixture = project / "fixture.py"; fixture.write_text("from pathlib import Path\nimport sys\noutput = Path(sys.argv[1])\noutput.parent.mkdir(parents=True, exist_ok=True)\noutput.write_text('repair_success=1\\n', encoding='utf-8')\nprint('repair_success=1')\n", encoding="utf-8")
        output_artifact = project / "artifacts" / "execution.txt"; output_artifact.parent.mkdir()
        if output_artifact.exists(): raise RuntimeError("declared execution output must start absent")
        execution_path = project / "artifacts" / "execution_record.json"; execution = evidence_anchor.execution_record(execution_path, [sys.executable, str(fixture), str(output_artifact)], cwd=project, output_paths=[output_artifact], environment={"network": False, "fixture": "public-v1"})
        if execution["status"] != "PASS" or execution["exit_status"] != 0: raise RuntimeError("fixture command failed output provenance")
        ...
        literature_runtime.verify_identity(literature_registry, "SRC-001", {"title": "Fixture prior work", "authors": ["Fixture Author"], "year": 2024, "venue": "Fixture Archive", "stable_identifier": "fixture:001"})
        retrieval = literature_runtime.record_retrieval(literature_registry, "SRC-001", literature_file, retrieval_method="local-open", source_uri="fixture://literature.txt", inspection_actor="smoke-retriever")
        claim_check = literature_runtime.verify_claim(literature_registry, "SRC-001", "C1", "PARTIALLY_SUPPORTS", "line 1", retrieval_record_id=retrieval["retrieval_id"], retrieval_method="local-open", source_uri="fixture://literature.txt", inspection_actor="smoke-inspector", checker="smoke-checker")
        if claim_check["status"] != "PASS" or claim_check["verification_status"] != "CLAIM_RELATION_VERIFIED": raise RuntimeError("literature claim verification failed")
        ...
        eval_runner = _load_eval_runner(); cases = project / "smoke_cases.json"; _write(cases, {"schema_version": 1, "skill_version": SKILL_VERSION, "cases": [{"id": "SMOKE-ROUTING", "type": "harness", "category": "routing", "prompt": "Start from a vague student idea.", "fixture": {}, "required_behaviors": ["creates a beginner brief", "runs feasibility"], "forbidden_behaviors": ["promises acceptance"], "required_artifacts": ["brief"]}]}); prepared = project / "eval"; eval_runner.prepare(cases, prepared); answer = project / "answer.txt"; answer.write_text("The system creates a beginner brief and runs feasibility before choosing an RQ. Findings remain evidence-bounded.", encoding="utf-8"); eval_runner.run_record(prepared / "manifest.json", "SMOKE-ROUTING", answer, model="deterministic-harness", host="local", reasoning_mode="harness", network=False, tools=[]); eval_runner.score(cases, prepared / "runs", project / "score.json"); eval_report = eval_runner.report(project / "score.json")
        anchors = json.loads((project / ".research-state" / "evidence_ledger.json").read_text(encoding="utf-8")).get("anchors", [])
        anchor_count = len(anchors)
        execution_anchor_checks = [evidence_anchor.deep_validate_anchor(item, project) for item in anchors if item.get("execution_record_id")]
        execution_anchor_status = "PASS" if execution_anchor_checks and all(item["status"] == "PASS" for item in execution_anchor_checks) else "FAIL"
        result = {"status": "PASS" if graph_check["status"] == "PASS" and graph_check_after["status"] == "PASS" and rebuild["status"] == "PASS" and eval_report["status"] == "PASS" and execution["status"] == "PASS" and execution_anchor_status == "PASS" and claim_check["verification_status"] == "CLAIM_RELATION_VERIFIED" and anchor_count >= 19 and route_result["status"] == "PASS" else "FAIL", "skill_commit": tested_commit, "skill_version": SKILL_VERSION, "evaluation_class": "HARNESS_SELF_TEST", "project_fixture": "public synthetic fixture", "workflow": ["orientation", "brief", "literature_discovery", "identity_verification", "claim_support_verification", "innovation", "feasibility", "experiment_planning", "protocol_freeze", "executable_command", "observed_output", "evidence_anchor", "analysis", "figure_table", "writing", "validation", "review", "revision", "author_handoff"], "node_evidence": {item["anchor_id"]: item["artifact_type"] for item in anchors}, "execution_anchor_sources": {item["anchor_id"]: item["source_artifact"] for item in anchors if item.get("execution_record_id")}, "graph_events": graph_check_after.get("event_count", 0), "anchor_count": anchor_count, "execution_record": {"status": execution["status"], "exit_status": execution["exit_status"], "stdout_sha256": execution["stdout_sha256"], "stderr_sha256": execution["stderr_sha256"], "outputs": execution["outputs"]}, "execution_anchor_verification": execution_anchor_status, "graph_validation": graph_check_after, "graph_rebuild": rebuild, "planner": planner_result, "router": route_result, "method_router": method_result, "literature": {"status": claim_check["verification_status"], "retrieval_record_id": retrieval["retrieval_id"], "region_sha256": claim_check["region_sha256"]}, "behavior_eval": eval_report, "model_behavior": "NOT_RUN; only harness self-test was executed", "privacy": "sanitized public output", "note": "True local fixture workflow; no scientific publication or venue claim."}
